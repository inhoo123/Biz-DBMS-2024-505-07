...
특히 java 라던가 다른 곳에서 DBMS 에 접속하는 프로그래밍을 수행할때는 문재를 일으킬 수 있다.
*/
...
/*
오라클 12c 이후 버전에서 영문+숫자 형식의 일번적인 사용자 ID 를 설정하기 위하여
...
-- 새로 생선된 user1 은 아무런 권한이 없어서, 로그인 조차 할수 없다
-- user1 에게 여러가지 권한을 부여해야만 DBMS 에 접근하여 업무를 수행할수 있다.
/*
권한 부여 정책
생성된 USER(user1)에게 로그인, 테이블생성, CRUD, 기타 여라가지 권한을 일일히 부여하는 것이 실무상 원칙
