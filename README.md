...
- 추상화된 DB Table 을 Diagram 으로 표현하는 설계
  모델링
- 개념모델링에서 그리는 Diagram을 `Entity-Relation
Diagram`, 줄여서 `ERD` 또는 `ER-Diagram`이라고 한다.
- `Entity` : DBMS 에서 Table 에 해당하는 개념
- `Attribute` : Table 에 소속 각 컬럼
- `Relation` : `Entity`와 `Entity` 가 서로 연결된
  상태의 Table

## DB 논리적 모델링

- 자연어 모델링된 데이터 정보를 물리적 모델링하기
  전에 자연어로 칼럼을 설정해보는 단계
