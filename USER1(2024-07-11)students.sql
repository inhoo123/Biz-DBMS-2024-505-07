...
VARCHAR2 : 순수 영, 숫자형 문자열,가변문자열,저장하는 문자열이 칼럼의 크기보다 작으면
    공백을 제거하여 공간 낭비를 막는형식
   
nVARCHAR2 : UTF-8 방식 영,숫자,한글 등 2byte 문자열을 효과적으로 저장히기 위한 type   

지금 현재의 DBMS 들은 대체로 영문, 한글 문자열을 특별히 구분하지 않고 처리를 한다.
현업에서 사용하는 오라클 DBMS 는 지금도 7,8,9 정도의 버전을 많이 사용한다

오라클 9 이하의 버전에 문자열의 type VARCHAR2, nVARCHAR2 로 명확히 구븐을 하는데
...
혼란을 준다
현재 버전에서 VARCHAR2 와 nVARCHAR2 를 구분하는것은 호환성을 위한 것이다
...
/*
INSERT INTO 를 실행할때 전체 칼럼에 일치하는 데이터를 추가할때
INSERT 되는 칼럼 리스트를 생략할수 있지만,
이는 데이터 무결성을 해칠 수 있는 아주 위험한 코드이다 절대 사용금지
*/
...
--아래 INSERT 코드는 절대 사용하지 말자
INSERT INTO tbl_student
VALUES ('2400001','홍길동','정보통신','3','01011111111');

/*
학생테이블(tbl_student)에서 이몽룡 학생의 전화번호를
010-3333-3333 에서 010-3333-3456 으로 변경하는 SQL 을 작성
*/
--st_name 칼럽은 PK 도 아니며, UNIQUE 칼럼도 아니다
--이러한 일반적인 칼럼은 WHERE 절에 사용하여 UPDATE,DELETE 를 실행하면
--데이터 무결성을 보장 할수 없다

SELECT *
FROM tbl_student
WHERE st_name = '이몽룡' AND st_tel = '010-3333-3456';
--UPDATE, DELETE 를 실행하고자 하는 데이터의 PK 칼럼 값을 반드시 먼저 확인 한 후
--PK 칼럽을 WHERE 절에 사용하여 실행해야 한다.
UPDATE tbl_student
